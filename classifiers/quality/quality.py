...
OUTPUT_DIRECTORY = Path(f"outputs/quality-predictor-{JOB_ID}/")
...
logger = logging.getLogger("quality-prediction")
...
def plot_confusion_matrix(
    ...
    cbar.set_label("Proportion of Actual Ratings (Normalized by Predicted Rating)")
    ...
    ax.set(
        ...
        ylabel="Actual Quality Rating",
        xlabel="Predicted Quality Rating",
    )
...
def main(dataset_path: Path, model_name: str, output_path: Path) -> None:
    """Fine-tunes DistilBERT to predict quality scores of a model"""
    ...
    logger.info(
        classification_report(
            labels,
            preds,
            target_names=[f"score_{i}" for i in range(NUM_SCORES)],
            zero_division=0,
        )
    )
    ...
    plot_confusion_matrix(
        labels,
        preds,
        class_names=[f"{i+1}" for i in range(NUM_SCORES)],
